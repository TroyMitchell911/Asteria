...
void app_clock_main_get_current_time(app_clock_time_t *t)
{
	if (t) {
		...
		ms = (uint64_t)elp * 1000 / RT_TICK_PER_SECOND +
		     total_milliseconds;
		cur_seconds = ms / 1000;
...
static void app_clock_main_select(uint16_t clock_idx)
{
	...
	if (clock_idx > 0)
		...
		left_clock_idx =
			p_app_clock_main
				->app_clock_list_len; //invalid left clock
	...
	if (right_clock_idx > p_app_clock_main->app_clock_list_len)
		right_clock_idx =
			p_app_clock_main
				->app_clock_list_len; //invalid right clock
	...
	rt_list_for_each(pos, (&p_app_clock_main->list))
	{
		...
		if ((i != clock_idx) && (i != left_clock_idx) &&
		    (i != right_clock_idx)) {
...
static void tileview_event_cb_t(lv_event_t *event)
{
	...
	case LV_EVENT_VALUE_CHANGED: {
		...
		if (gui_app_is_actived(
			    APP_ID)) //value_changed could be sent after app paused
	...
	(LV_HOR_RES_MAX ==     \
	 240) //Active neighbor clock may cause malloc mem failure in high resolution
...
static void app_clock_main_init(void)
{
	...
	rt_uint16_t
		last_active_clock_bak; /*when tileview created, LV_EVENT_VALUE_CHANGED (idx is 0) will be send */
	...
	p_app_clock_main->p_tileview_valid_pos = (lv_point_t *)rt_malloc(
...
static void app_clock_load_one_dyn_wf(const char *module_name, const char *path)
{
	...
	if (mod && (old_len != p_app_clock_main->app_clock_list_len)) {
		clk_desc = rt_list_tail_entry(&p_app_clock_main->list,
					      app_clock_desc_t, node);
		clk_desc->mod = mod;
	}
}
		...
		if (stat(full_path, ent_stat) == 0) {
			if (!S_ISDIR(ent_stat->st_mode)) {
				...
				if (('m' == dir_entry->d_name[name_len - 1]) &&
				    ('.' ==
				     dir_entry->d_name[name_len -
						       2])) /* ending with .m */
				{
					/* remove suffix .m */
					dir_entry->d_name[name_len - 2] = 0;
					app_clock_load_one_dyn_wf(
...
void app_clock_reset_time(void)
{
	...
	latched_clock = clock();
	total_milliseconds =
		(((time_info->tm_hour * 60) + time_info->tm_min) * 60 +
		 time_info->tm_sec) *
		1000;
...
static void on_start(void)
{
	/* init list*/
	p_app_clock_main =
		(app_clock_main_t *)rt_malloc(sizeof(app_clock_main_t));
