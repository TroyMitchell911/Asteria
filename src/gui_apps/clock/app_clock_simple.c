...
static void app_clock_simple_redraw(lv_timer_t *task)
{
	...
	lv_img_set_offset_x(
		...
		-(((seconds * 1000 + milli_seconds) / 33) % LV_HOR_RES_MAX));
	lv_img_set_offset_y(
		p_clk_simple->masked_img_obj,
		-(((seconds * 1000 + milli_seconds) / 33) % LV_VER_RES_MAX));
#endif /* ENABLE_MASKED_IMAGE */
	memcpy(&p_clk_simple->last_redraw_time, &current_time,
...
static void init_clock_hands_img(void)
{
	...
	if (NULL == p_clk_simple->hor_cache) {
		p_clk_simple->hor_cache = app_cache_copy_alloc(
			LV_EXT_IMG_GET(clock_simple_hour_hand), ROTATE_MEM);
		RT_ASSERT(p_clk_simple->hor_cache != NULL);
		lv_img_set_src(p_clk_simple->hour_hand,
			       p_clk_simple->hor_cache);
	}

	if (NULL == p_clk_simple->min_cache) {
		p_clk_simple->min_cache = app_cache_copy_alloc(
			LV_EXT_IMG_GET(clock_simple_minute_hand), ROTATE_MEM);
		RT_ASSERT(p_clk_simple->min_cache != NULL);
		lv_img_set_src(p_clk_simple->minute_hand,
			       p_clk_simple->min_cache);
	}

	if (NULL == p_clk_simple->sec_cache) {
		p_clk_simple->sec_cache = app_cache_copy_alloc(
			LV_EXT_IMG_GET(clock_simple_second_hand), ROTATE_MEM);
		RT_ASSERT(p_clk_simple->sec_cache != NULL);
		lv_img_set_src(p_clk_simple->second_hand,
			       p_clk_simple->sec_cache);
	}

#endif

	lv_obj_align(p_clk_simple->hour_hand, LV_ALIGN_CENTER, 0,
		     (clock_simple_hour_hand.header.h >> 1) - 157);
	lv_obj_align(p_clk_simple->minute_hand, LV_ALIGN_CENTER, 0,
		     (clock_simple_minute_hand.header.h >> 1) - 186);
	lv_obj_align(p_clk_simple->second_hand, LV_ALIGN_CENTER, 0,
		     (clock_simple_second_hand.header.h >> 1) - 230);
...
static void deinit_clock_hands_img(void)
{
	...
	lv_img_set_src(p_clk_simple->hour_hand,
		       LV_EXT_IMG_GET(clock_simple_hour_hand));
	lv_img_set_src(p_clk_simple->minute_hand,
		       LV_EXT_IMG_GET(clock_simple_minute_hand));
	lv_img_set_src(p_clk_simple->second_hand,
		       LV_EXT_IMG_GET(clock_simple_second_hand));
...
static rt_int32_t resume_callback(void)
{
	...
	if (NULL == p_clk_simple->redraw_task) {
		p_clk_simple->redraw_task =
			lv_timer_create(app_clock_simple_redraw, 30, (void *)0);
	}
...
static void bg_img_event_cb(lv_event_t *e)
{
	...
	if (code == LV_EVENT_CLICKED) {
		if (lv_obj_is_visible(p_clk_simple->masked_img_obj))
			lv_obj_add_flag(p_clk_simple->masked_img_obj,
					LV_OBJ_FLAG_HIDDEN);
		else
			lv_obj_clear_flag(p_clk_simple->masked_img_obj,
					  LV_OBJ_FLAG_HIDDEN);
	}
}
...
{
	p_clk_simple =
		(app_clock_simple_t *)rt_malloc(sizeof(app_clock_simple_t));
	...
	lv_img_set_src(p_clk_simple->masked_img_obj,
		       LV_EXT_IMG_GET(clock_simple_bg));
	lv_img_set_size_mode(p_clk_simple->masked_img_obj,
			     LV_IMG_SIZE_MODE_REAL);
	//Set same size as mask
	lv_obj_set_size(p_clk_simple->masked_img_obj, a8_mask_300x200.header.w,
			a8_mask_300x200.header.h);
