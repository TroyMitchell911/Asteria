...
static lv_point_t *get_icon_pivot(uint32_t row_idx, uint32_t col_idx)
{
	...
	return &(app_mainmenu_ctx
			 .icon_pivot[col_idx * MAX_APP_ROW_NUM + row_idx]);
}
			      ...
			      const void *img, uint8_t row_idx, uint8_t col_idx)
{
	...
	lv_img_set_src(
		icon, img); //lv_img_set_src(icon, LV_EXT_IMG_GET(img_mail)); //
				 ...
				 float *p_float_pivot_r)
{
	...
	if (get_icon_transform_param(float_x, float_y, float_icon_r, &float_x,
...
#endif /* DEBUG_APP_MAINMENU_DISPLAY_ICON_PARAM */
		{
			...
			limit_round2(LIMIT_ROUND_RADIUS, &scr_center, &float_x,
				     &float_y, &float_icon_r, pivot_r);
			limit_square(&parent_area, &float_x, &float_y,
				     &float_icon_r);

#else //round screen
			limit_round2(LV_HOR_RES >> 1, &scr_center, &float_x,
				     &float_y, &float_icon_r, pivot_r);
	...
	if (float_icon_r >= (ICON_OUTER_RADIUS - ICON_INNER_RADIUS)) {
		float_icon_r =
			float_icon_r - (ICON_OUTER_RADIUS - ICON_INNER_RADIUS);
	} else {
		float_icon_r = 0;
	}
#endif /* 0 */
		...
		//Move icon
		{
			...
			lv_obj_move_to(
				...
				((lv_coord_t)(pi_x_10p8 >> 8)) +
					lv_obj_get_scroll_x(
						app_mainmenu_ctx.pg_obj),
				((lv_coord_t)(pi_y_10p8 >> 8)) +
					lv_obj_get_scroll_y(
						app_mainmenu_ctx.pg_obj));

#ifndef DISABLE_LVGL_V8
			lv_img_set_x_frac(obj, (uint16_t)((pi_x_10p8 << 8)) &
						       0xFFFF);
			lv_img_set_y_frac(obj, (uint16_t)((pi_y_10p8 << 8)) &
						       0xFFFF);
#endif
		}
	...
	for (row_idx = 0; row_idx < MAX_APP_ROW_NUM; row_idx++)
		for (col_idx = 0; col_idx < MAX_APP_COL_NUM; col_idx++) {
			...
			if (icon) {
				...
				uint16_t zoom = (uint16_t)(ICON_IMG_WIDTH *
							   256 / img_w);

				//rt_kprintf("init icon [%d,%d]  %d\n", x, y, zoom);

				lv_obj_set_pos(icon, x - (img_w >> 1),
					       y - (img_h >> 1));
				lv_img_set_pivot(icon, (img_w >> 1),
						 (img_h >> 1));
...
static void icon_event_callback(lv_event_t *e)
{
	...
	if (app_mainmenu_ctx.scroll_actived &&
		...
		lv_obj_add_flag(app_mainmenu_ctx.pg_obj,
...
#ifndef DISABLE_LVGL_V8
			lv_event_send(app_mainmenu_ctx.pg_obj,
				      LV_EVENT_SCROLL_END, NULL);
#else
			lv_obj_send_event(app_mainmenu_ctx.pg_obj,
					  LV_EVENT_SCROLL_END, NULL);
#endif
		}
...
static void page_event_callback(lv_event_t *e)
{
	...
	if (LV_EVENT_PRESSED == event) {
		...
		app_mainmenu_ctx.anim_obj = NULL;
		lv_obj_clear_flag(app_mainmenu_ctx.pg_obj,
				  LV_OBJ_FLAG_SCROLLABLE);
		...
		if (app_mainmenu_ctx.anim_obj) {
			char *cmd = (char *)lv_obj_get_user_data(
				app_mainmenu_ctx.anim_obj);

			if (cmd) {
				rt_kprintf(
					"app mainmenu icon click anim cbk\n");
				gui_app_run(cmd);
			}
			app_mainmenu_ctx.anim_obj = NULL;
		}
					 ...
					 lv_obj_t *page)
{
	...
	} else if (0 == strcmp(APP_ID, builtin_app->id)) //skip main menu icon
	{
		...
		add_app_icon(page, builtin_app->id, builtin_app->icon, row,
			     col);
		idx++;
	}
...
static void app_mainmenu_read_app_icons(lv_obj_t *page)
{
	...
	if (p_builtin_app) {
		do {
			//Fix 1st icon for clock
			idx = reorder_clock_icon(idx, clock_idx, p_builtin_app,
						 page);
			p_builtin_app =
				gui_builtin_app_list_get_next(p_builtin_app);
		} while (p_builtin_app);

		while (1) {
			//Fix 1st icon for clock
			p_builtin_app =
				gui_script_app_list_get_next(p_builtin_app);
...
#if 1 //for demo, full fill screen
	{
		...
		const void *dummy_icons[] = {
			LV_EXT_IMG_GET(img_mail),
			LV_EXT_IMG_GET(img_alarm_2),
			LV_EXT_IMG_GET(img_workout),
			LV_EXT_IMG_GET(img_alarm),
			LV_EXT_IMG_GET(img_mail),
			LV_EXT_IMG_GET(img_alarm_2),
			LV_EXT_IMG_GET(img_workout),
			LV_EXT_IMG_GET(img_alarm),
		...
		for (i = 0; i < sizeof(dummy_icons) / sizeof(dummy_icons[0]);
		     i++, idx++) {
			layout_get_icon_col_row_by_idx(idx, &col, &row);
			lv_obj_t *p_obj = add_app_icon(
				page, "none", dummy_icons[i], row, col);
	...
	for (row_idx = 0; row_idx < MAX_APP_ROW_NUM; row_idx++)
		for (col_idx = 0; col_idx < MAX_APP_COL_NUM; col_idx++) {
			...
			if (*get_icon_obj(row_idx, col_idx)) {
				float float_x =
					(float)get_icon_pivot(row_idx, col_idx)
						->x;
				float float_y =
					(float)get_icon_pivot(row_idx, col_idx)
						->y;
				...
				if (app_mainmenu_ctx.zoom < 1) {
					...
					zoom = zoom * zoom;
					float_x = MM_ZOOM(LV_HOR_RES_MAX >> 1,
							  float_x, zoom);
					float_y = MM_ZOOM(LV_VER_RES_MAX >> 1,
							  float_y, zoom);
					float_icon_w *= zoom;
					float_icon_h *= zoom;
				}
				if (app_mainmenu_ctx.zoom > 0) {
					...
					float float_icon_w_before =
						float_icon_w;
					float float_icon_h_before =
						float_icon_h;

					//calculate draw pivot and radius
					if (layout_icon_transform(
						    row_idx, col_idx, &float_x,
						...
						if ((app_mainmenu_ctx.zoom >
						     0) &&
						    (app_mainmenu_ctx.zoom <=
						     1)) {
							zoom = app_mainmenu_ctx
								       .zoom;
							float_x = MM_ZOOM(
								float_x_before,
								float_x, zoom);
							...
							float_icon_w = MM_ZOOM(
								float_icon_w_before,
								float_icon_w,
								zoom);
							float_icon_h = MM_ZOOM(
								float_icon_h_before,
								float_icon_w,
								zoom);
						}
				...
				app_mainmenu_draw_icon(
					*get_icon_obj(row_idx, col_idx),
					float_x, float_y, float_icon_w,
					float_icon_h);

#ifdef DEBUG_APP_MAINMENU_DISPLAY_ICON_COORDINATE
				app_mainmenu_draw_icon_label(
					row_idx, col_idx, float_x, float_y,
					float_icon_w, float_icon_h, pivot_r);
#endif
			}
		}
...
static void mm_trans_anim_init(void)
{
	...
	gui_app_trans_anim_init_cfg(&enter_anim_cfg,
